...
const apiErrorMessage = ( error ) =>
	error?.message ||
	__(
		'WP Branches For Post could not complete the request.',
...
const stateLabel = ( state ) => {
	const labels = {
		...
		informational: __(
			'Ready · original identity changed',
			'wp-branches-for-post'
		),
		rebase_available: __(
			'Original updated · safe to refresh branch',
			'wp-branches-for-post'
		),
...
function ChangeList( { title, paths, danger = false } ) {
	...
	return el(
		...
		{
			className: danger
				? 'wbfp-review-group wbfp-review-group--danger'
				: 'wbfp-review-group',
		},
...
function BranchPanel() {
	...
	const refreshStatus = async () => {
		...
		const result = await apiFetch( {
			path: `/wbfp/v1/posts/${ editor.postId }/status`,
		} );
		setStatus( result );
		return result;
	};
	...
	useEffect( () => {
		...
		apiFetch( { path: `/wbfp/v1/posts/${ editor.postId }/status` } )
			...
			.catch(
				( requestError ) =>
					active && setError( apiErrorMessage( requestError ) )
			);
		return () => {
			active = false;
		};
	...
	const createBranch = async () => {
		if ( editor.dirty ) {
			setError(
				__(
					'Save or discard the current edits to the original before creating a branch. This prevents unpublished editor changes from being mistaken for the branch baseline.',
					'wp-branches-for-post'
				)
			);
			return;
	...
	const mergeBranch = async ( force = false ) => {
		const reviewedToken = status?.review_token || '';
		setBusy( true );
		setError( '' );
		try {
			await saveBranchIfNeeded();
			...
			const reviewChanged =
				! reviewedToken ||
				! fresh?.review_token ||
				fresh.review_token !== reviewedToken;
			const blocked = [
				'conflict',
				'changed',
				'unknown',
				'missing',
			].includes( fresh?.conflict );
			if ( reviewChanged || ( blocked && ! force ) ) {
				setReviewOpen( true );
				setError(
					__(
	...
	if ( status?.type === 'original' ) {
			...
			el(
				...
				el(
					...
					el(
						'span',
						{
							className: `wbfp-state wbfp-state--${ branch.conflict }`,
						},
				...
				el(
					...
					sprintf(
						...
						__(
							'%1$d branch changes · %2$d conflicts',
							'wp-branches-for-post'
						),
		...
		content = el(
			...
			el(
				...
				null,
				__(
					'Edit safely in a separate draft. The published post stays unchanged until you merge the branch.',
					'wp-branches-for-post'
				)
			),
			editor.dirty &&
				el(
					...
					__(
						'This original has unsaved editor changes. Save or discard them before creating a branch so the baseline matches the saved public post.',
						'wp-branches-for-post'
					)
				),
			status.can_create &&
				el(
					Button,
					{
						...
						disabled: busy || editor.dirty || editor.saving,
					},
					busy
						? __( 'Creating…', 'wp-branches-for-post' )
						: __( 'Create branch', 'wp-branches-for-post' )
				),
			status.branches?.length > 0 &&
				el(
					'div',
					{ className: 'wbfp-existing-branches' },
					el(
						'strong',
						null,
		...
		const hasBranchChanges =
			status.legacy || ( review.branch_changes?.length || 0 ) > 0;
		const reviewPanel =
			reviewOpen &&
			el(
				'div',
				...
				el(
					'div',
					{ className: 'wbfp-review__title' },
					__( 'Merge review', 'wp-branches-for-post' )
				),
				status.legacy &&
					el(
						Notice,
				...
				el( ChangeList, {
					title: __( 'Branch changes', 'wp-branches-for-post' ),
					paths: review.branch_changes,
				} ),
				el( ChangeList, {
					title: __(
						'Original changes since branch creation',
						'wp-branches-for-post'
					),
					paths: review.original_changes,
				} ),
				el( ChangeList, {
					title: __( 'Conflicts', 'wp-branches-for-post' ),
					paths: review.conflicts,
					danger: true,
				} ),
				el( ChangeList, {
					title: __(
						'Original identity changes preserved by merge',
						'wp-branches-for-post'
					),
					paths: review.informational_changes,
				} ),
				el( ReviewValues, { values: status.review_values } ),
				! status.legacy &&
					! review.branch_changes?.length &&
					el(
						'p',
						{ className: 'wbfp-meta' },
						__(
							'No mergeable branch changes are currently detected.',
							'wp-branches-for-post'
						)
					),
				el(
					...
					status.can_merge &&
						...
						el(
							...
							{
								variant: 'primary',
								onClick: () => mergeBranch( false ),
								disabled: busy || editor.saving,
							},
							busy
								? __(
										'Saving and merging…',
								...
								: __(
										'Save & merge into original',
										'wp-branches-for-post'
									)
						),
					status.can_merge &&
						hardConflict &&
						status.can_force_merge &&
						state !== 'missing' &&
						el(
							Button,
							{
							...
							busy
								...
								: __(
										'Force merge reviewed conflicts',
										'wp-branches-for-post'
									)
						)
				)
		...
		if ( state === 'rebase_available' ) {
			stateNotice = el(
				...
				__(
					'The original has newer, non-conflicting changes. You can update this branch first, or review and merge while preserving those original changes.',
					'wp-branches-for-post'
				)
			);
			...
			stateNotice = el(
				...
				{ status: 'info', isDismissible: false },
				__(
					'The original identity changed (for example status, slug, author, or parent). Those changes are preserved and are not overwritten by this branch.',
					'wp-branches-for-post'
				)
			);
			...
			stateNotice = el(
				...
				{ status: 'warning', isDismissible: false },
				__(
					'The branch and original changed the same data differently. Review the conflicts before deciding whether to force merge.',
					'wp-branches-for-post'
				)
			);
			...
			stateNotice = el(
				...
				{ status: 'warning', isDismissible: false },
				__(
					'This older branch cannot provide a precise three-way comparison. Review the original manually before forcing a merge.',
					'wp-branches-for-post'
				)
			);
		...
		content = el(
			...
			el(
				...
				null,
				__(
					'This draft is isolated from the public original.',
					'wp-branches-for-post'
				),
				status.original_edit_url &&
					el(
						Fragment,
						null,
						' ',
						el(
							'a',
							{ href: status.original_edit_url },
							__( 'Open original', 'wp-branches-for-post' )
						)
					)
			),
			status.creator &&
			...
			el(
				'div',
				{ className: `wbfp-state wbfp-state--${ state }` },
				stateLabel( state )
			),
			status.synced_pattern_count > 0 &&
				el(
					Notice,
					{ status: 'warning', isDismissible: false },
					sprintf(
						...
						__(
							'This branch references %d synced pattern(s). Editing a synced pattern is global in WordPress and is not isolated by the branch.',
							'wp-branches-for-post'
						),
			...
			el(
				'div',
				{ className: 'wbfp-preview-links' },
				status.branch_preview_url &&
					el(
						'a',
						{
							href: status.branch_preview_url,
							target: '_blank',
							rel: 'noreferrer',
						},
						__( 'Preview branch', 'wp-branches-for-post' )
					),
				status.original_preview_url &&
					el(
						'a',
						{
							href: status.original_preview_url,
							target: '_blank',
							rel: 'noreferrer',
						},
						__( 'View original', 'wp-branches-for-post' )
					)
			),
			status.can_rebase &&
				el(
					...
					{
						variant: 'secondary',
						onClick: rebaseBranch,
						disabled: busy || editor.saving,
					},
					busy
						? __( 'Updating…', 'wp-branches-for-post' )
						: __(
			...
			el(
				...
				el(
					...
					reviewOpen
						? __( 'Hide merge review', 'wp-branches-for-post' )
						: __( 'Review changes', 'wp-branches-for-post' )
				),
				status.can_discard &&
					el(
						Button,
	...
	const confirmation =
		...
		el(
			...
			el(
				...
				confirmAction === 'force'
					? __(
							'Force merge will prefer branch values for the listed conflicts. Non-conflicting newer work on the original is still preserved.',
							'wp-branches-for-post'
						)
					: __(
							'The branch will be moved to Trash. The public original will not be changed.',
							'wp-branches-for-post'
						)
			),
			el(
	...
	return el(
		...
		{
			name: 'wbfp-branch',
			title: __( 'Post Branch', 'wp-branches-for-post' ),
		},
		error &&
			el(
				Notice,
